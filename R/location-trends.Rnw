<<data-location-trend, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6, out.width=".95\\linewidth", fig.align="center", fig.cap="In order to illustrate the relationship between object location and feature values, a thinned out scatterplot (only a randomly sampled 1\\% of datapoints are shown) of mean actin intensity against distance from image cetner is reproduced alognside a trendline as calculated by gam of the CRAN mgcv package. An approximately linear trend is discernible which can be found in many feature types.", fig.scap="Scatterplot visualization showing mean actin intensity against object distance from image center alongside a trend line.">>=
