<<analysis-glm1, eval=TRUE, echo=FALSE, results='asis', message=FALSE, warning=FALSE>>=
...
%\newcommand{\knitrDataGlm1PerSepScram}{\Sexpr{pasteAnd(names(perfect.sep$Scrambled)[which(perfect.sep$Scrambled)])}}
%\newcommand{\knitrDataGlm1PerSepMtor}{\Sexpr{pasteAnd(names(perfect.sep$MTOR)[which(perfect.sep$MTOR)])}} 
%\newcommand{\knitrDataGlm1PerSepTgfbr1}{\Sexpr{pasteAnd(names(perfect.sep$TGFBR1)[which(perfect.sep$TGFBR1)])}}
%\newcommand{\knitrDataGlm1PerSepRipk4}{\Sexpr{pasteAnd(names(perfect.sep$RIPK4)[which(perfect.sep$RIPK4)])}}
%\newcommand{\knitrDataGlm1PerSepPik3r3}{\Sexpr{pasteAnd(names(perfect.sep$PIK3R3)[which(perfect.sep$PIK3R3)])}}
