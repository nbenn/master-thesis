<<who-deaths_by-disease, eval=TRUE, echo=FALSE, fig.width=12, fig.height=6, fig.cap="Relative frequencies deadly infectious diseases for 2012 by World Bank income groups. Binning is based on Gross National Income (GNI; see figure \\ref{fig:who-deaths_top-causes}). The data was obtained from the \\cite{WHO2012}.", fig.scap="Relative frequencies deadly infectious diseases for 2012 by World Bank income groups">>=
...
ggplot(aggr, aes(x=Region, y=scaled, fill=Cause)) +
  ...
  theme(axis.title.y=element_blank(), legend.key.height=unit(12, "mm"),
        plot.margin=unit(c(c(0, 3, 0, 3)), units="mm"))
@
